...
switch (_callbackTarget) do {
    ...
    case "SUPPLY":
    {
		switch (_callbackType) do {
			...
			case CALLBACK_SHOULD_CANCEL_PLACEMENT: {
				if (!(player inArea "Synd_HQ")) exitWith {
					[true, "You need to be close to your HQ to be able to retrieve a suppy crate"];
				};
				[false];
			};
			...
			case CALLBACK_VEH_IS_VALID_LOCATION: {
				private _pos = _callbackParams select 0;
				private _maxDist = 50;
				if (_pos distance2d (getMarkerPos "Synd_HQ") > _maxDist) exitWith
				{
					[false, format ["This supply box must be placed within %1m of the flag", _maxDist]];
				};
				[true];
			};
			...
			case CALLBACK_CAN_PLACE_VEH: {
				if (!(player inArea "Synd_HQ")) exitWith
				{
					[false, "You need to be close to your HQ to be able to retrieve a suppy crate"];
				};
				if ([player,300] call A3A_fnc_enemyNearCheck) exitWith
				{
					[false, "You cannot retrieve a supply crate with enemies nearby"];
				};
				[true];
			};
